...
//#define kEdgePanGestureMargin 10.0f
...
- (void)_updateInsetsWithContentSize:(CGSize)contentSize animated:(BOOL)animated
{
    ...
    if ( animated ) //[UIView animateWithDuration:kAnimationDuration animations:block];
        [UIView animateWithDuration:kAnimationDuration delay:0.0 usingSpringWithDamping:1 initialSpringVelocity:0 options:0 animations:block completion:nil];
    else block();
}
...
- (void)setZoomScale:(CGFloat)scale animated:(BOOL)animated completion:(void(^)(BOOL finished))block
{
    ...
    [UIView animateWithDuration:kAnimationDuration delay:0.0 usingSpringWithDamping:1 initialSpringVelocity:0 options:UIViewAnimationOptionBeginFromCurrentState
    animations:^
    {
...
- (void)viewDidLayoutSubviews
{
    ...
    [self _scrollViewSetDefaultScalingAnimated:NO completion:^(BOOL finished)
    {
...
/*
//NO CAL PERQUE EL DID LAYOUT JA HO FA  ( amb aixo i el sense el didLayout la rotacio es millor, pero el did contempla la situacio inicial)
...
//{
//
//    UIInterfaceOrientation orientation = [self orientationFromTransform: [coordinator targetTransform]];
//    UIInterfaceOrientation oldOrientation = [[UIApplication sharedApplication] statusBarOrientation];
//    // will rotate code goes here
//    // ...
//    
...
- (CGFloat)_getDefaultScaling
{
    ...
    // correct the scale for iphone witdth (particularly relevant for landscape 3.5' screens)
    if ( IS_IPHONE && (contentSize.width > boundsSize.width ) )
    {
        scale = (boundsSize.width-leftMargin-rightMargin)/contentSize.width;
    }
...
- (void)_scrollViewSetDefaultScalingAnimated:(BOOL)animated completion:(void(^)(BOOL finished))block
{
    ...
    if ( IS_IPHONE )
    {
        ...
        _scrollView.maximumZoomScale = (_scalingEnabled ? 1.5 : defaultScaling);
    }
    else
    {
        _scrollView.minimumZoomScale = (_scalingEnabled ? 0.5 : defaultScaling);
...
//    
//    [_masterViewController setZoomScaleFactor:contentScale];
//}
...
/*
    AQUI NO CAL PERQUE JA HO FA DIDLAYOUTSUBVIEWS
    [self _scrollViewSetDefaultScalingAnimated:animated completion:^(BOOL finished)
    {
        [self _updateMasterScale];
    }];
...
- (void)_performTransitionToViewController:(UIViewController<SWZoomableViewController>*)inController
    animation:(SWMasterViewControllerPresentationAnimation)animation completion:(void (^)(BOOL finished))completion
{
    UIViewController *outController = _masterViewController;
    _masterViewController = inController;
    UIView *outputView = nil;
    
    [self _prepareMainViewSubframe];
    void (^transitionCompletion)(void) =
        [self _transitionFromViewController:outController toViewController:inController inView:_masterContentView removingViewSnapshot:&outputView];
    
    UIView *inControllerView = inController.view;
    UIView *outControllerView = outputView;
...
{
    if ( appear ) return [self _deployControllerView:controller.view inView:view];
    if ( disappear ) return [self _undeployControllerView:controller.view];
    return ^{};
}
...
- (void (^)(void))_deployControllerView:(UIView*)controllerView inView:(UIView*)view
{
...
// animation completion, or right after return in case of non-animated deployment.
- (void (^)(void))_undeployControllerView:(UIView*)controllerView
{
    if (!controllerView)
        return ^(void){};
...
- (void(^)(void))_transitionFromViewController:(UIViewController*)fromController toViewController:(UIViewController*)toController inView:(UIView*)view
{
    ...
    void (^deployCompletion)() = [self _deployControllerView:toController.view inView:view];
    
    [fromController willMoveToParentViewController:nil];
    void (^undeployCompletion)() = [self _undeployControllerView:fromController.view];
...
//- (void(^)(void))_transitionFromViewController:(UIViewController*)fromController toViewController:(UIViewController*)toController inView:(UIView*)view
//{
//    return [self _transitionFromViewController:fromController toViewController:toController inView:view outputView:nil];
//}


//// Containment Transition method. Returns a block to be invoked at the
//// animation completion, or right after return in case of non-animated transition.
//- (void(^)(void))_transitionFromViewController:(UIViewController*)fromController toViewController:(UIViewController*)toController inView:(UIView*)view outputView:(UIView**)outputView;
//{
//    if ( fromController == toController )
//        return ^(void){};
//    
//    UIView *outView = [fromController.view snapshotViewAfterScreenUpdates:NO];
//    [view insertSubview:outView aboveSubview:fromController.view];
//    if ( outputView ) *outputView = outView;
//    
//    [fromController willMoveToParentViewController:nil];
...
//    
//    void (^deployCompletion)() = [self _deployControllerView:toController.view inView:view];
//    void (^undeployCompletion)() = [self _undeployControllerView:outView];
//    
//    void (^completionBlock)(void) = ^(void)
//    {
//        undeployCompletion();
...
// Containment Transition method. Returns a block to be invoked at the
// animation completion, or right after return in case of non-animated transition.
- (void(^)(void))_transitionFromViewController:(UIViewController*)fromController toViewController:(UIViewController*)toController inView:(UIView*)view removingViewSnapshot:(UIView**)outputView;
{
    if ( fromController == toController )
        return ^(void){};
    ...
    if ( fromController )
    {
        // creem un snapshot del view que marxa i l'insertem a la jerarquia dels views
        UIView *fromControllerView = fromController.view;
        snapshotView = [fromControllerView snapshotViewAfterScreenUpdates:NO];
        if ( snapshotView ) [view insertSubview:snapshotView aboveSubview:fromControllerView];
        if ( outputView ) *outputView = snapshotView;

        // eliminem sense animacio el fronController (pero l'hem substituit per el snapshot)
        [fromController willMoveToParentViewController:nil];
        [fromControllerView removeFromSuperview];
        [fromController removeFromParentViewController];
    }
    
    // completem la transicio
    void (^transitionCompletionBlock)() = [self _transitionFromViewController:nil toViewController:toController inView:view];
    
    void (^completionBlock)(void) = ^(void)
    {
        transitionCompletionBlock();
        [snapshotView removeFromSuperview];  // <-- eliminem el snapshot de la jerarquia
    };
...
- (void)_handleRightRevealGestureStateChangedWithRecognizer:(UIPanGestureRecognizer *)recognizer
{
    ...
    else if ( xPosition > vRect.origin.x+vRect.size.width)
