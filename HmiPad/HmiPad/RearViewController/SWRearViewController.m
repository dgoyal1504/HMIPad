...
@interface SWRearViewController() <AppFilesModelObserver, AppModelDocumentObserver, AppUsersModelObserver, SWAppCloudKitUserObserver, UITableViewDataSource, UITableViewDelegate, LoginWindowControllerDelegate, SWRevealViewControllerDelegate>
...
@interface SWRearViewController()
{
    ...
    ControlViewCell *redeemCodeCell; // celda per accedir als settings
...
#if HMiPadDev
enum rowsInGeneralSection
{
    kCurrentAccountRow = 0,
    kSettingsRow,
    TotalRowsInGeneralSection,
        kRedeemCodeRow,
        kManageAccountsRow,
        kAutomaticLoginRow,
};
...
- (ControlViewCell*)currentAccountCell
{
    if ( currentAccountCell ==  nil )
    {
        ...
        [self _setDefaultSelectionStyleForCell:currentAccountCell];
        
        NSString *userName = [usersModel() currentUserName];
        [currentAccountCell setMainText:userName];
        [currentAccountCell.cellContentView setMainTextFont:[UIFont boldSystemFontOfSize:17]];
        
        UIImage *image = [[UIImage imageNamed:@"769-male.png"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
...
- (void)_setupCurrentAccountCell
{
    ...
    SWAppCloudKitUser *ckUser = cloudKitUser();
    UserProfile *profile = ckUser.currentUserProfile;

    BOOL isIcloudReady = profile.token != nil;
    if ( isIcloudReady )
    {
        NSString *userName = profile.username;
        mainText = userName.length > 0 ? userName : NSLocalizedString(@"iCloud user", nil);
        if ( userName.length > 0 )
        {
            NSString *email = profile.email;
            detailText = email.length > 0 ? email : NSLocalizedString(@"(no email)", nil);
        }
    }
    else
    {
        mainText = NSLocalizedString(@"(No iCloud user)", nil);
    }
    
    [currentAccountCell setMainText:mainText];
    [currentAccountCell setBottomText:detailText];
}
...
- (ControlViewCell*)automaticLoginCell
{
    if ( automaticLoginCell == nil)
    {
        ...
        [switchv=[automaticLoginCell switchv] setOn:[usersModel() automaticLogin]];  // agafa de preferences
        [switchv addTarget:self action:@selector(autoLoginSwitchChanged:) forControlEvents:UIControlEventValueChanged];
        //[switchv setOnTintColor:self.view.tintColor];
    }
...
- (ControlViewCell*)manageAccountsCell
{
    if ( manageAccountsCell == nil )
    {
        ...
        [self _setDefaultSelectionStyleForCell:manageAccountsCell];
        NSString *text = NSLocalizedString(@"Accounts",nil);
        [manageAccountsCell setMainText:text];
...
- (LabelViewCell*)currentProjectCellV
{
    if ( currentProjectCell == nil )
    {
        ...
        [self _setDefaultSelectionStyleForCell:currentProjectCell];
...
- (LabelViewCell*)currentProjectCell
{
    if ( currentProjectCell == nil )
    {
        ...
        UIImage *image = [[UIImage imageNamed:@"1013-hammer.png"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        UIImageView *imgView = [[UIImageView alloc] initWithImage:image];
        [imgView setTintColor:[UIColor whiteColor]];
        [imgView setContentMode:UIViewContentModeCenter] ;
        [imgView setFrame:CGRectMake(0, 0, 40, 40)];
...
- (ControlViewCell*)redeemCodeCell
{
    if ( redeemCodeCell == nil )
    {
        redeemCodeCell = [[ControlViewCell alloc] initWithReuseIdentifier:nil];
        [redeemCodeCell setAccessoryType:UITableViewCellAccessoryDisclosureIndicator];
        //[redeemCodeCell setSelectionStyle:UITableViewCellSelectionStyleBlue];
        [self _setDefaultSelectionStyleForCell:redeemCodeCell];
        [redeemCodeCell setMainText:NSLocalizedString(@"Redeem Activation Code",nil)];
    }
    return redeemCodeCell ;
...
- (ControlViewCell*)settingsCell
{
    if ( settingsCell == nil )
    {
        ...
        [self _setDefaultSelectionStyleForCell:settingsCell];
...
- (SwitchViewCell *)fileServerSwitchCell
{
    if ( fileServerSwitchCell == nil )
    {
        ...
        [(switchv = [fileServerSwitchCell switchv]) addTarget:self action:@selector(switchChanged:) forControlEvents:UIControlEventValueChanged];
    }
...
- (void)_setDefaultSelectionStyleForCell:(UITableViewCell*)cell
{
    [cell setSelectionStyle:UITableViewCellSelectionStyleDefault];
    UIView *selectView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 10, 10)];
    //selectView.backgroundColor = /*[UIColor blueColor];*/ UIColorWithRgb(TangerineSelectionColor);
    selectView.backgroundColor = self.view.tintColor;
...
#define MBColor [UIColor clearColor]
...
- (SWTableViewMessage *)messageTableEmpty
{
    if ( messageTableEmpty == nil )
    {
        if ( _tableView.style == UITableViewStylePlain )
        {
            ...
            [messageTableEmpty.messageViewLabel setTextColor:[UIColor whiteColor]];
            [messageTableEmpty.messageViewLabel setShadowColor:[UIColor blackColor]];
        }
        else
        {
...
//    [[self.navigationController navigationBar] addGestureRecognizer:toolbarController.panGestureRecognizer];
    ...
    [tableView setSectionHeaderHeight:34];
...
//            target:toolbarController action:@selector(leftOverlayPositionToggle)];
    ...
    [navBar addGestureRecognizer:toolbarController.panGestureRecognizer];
    ...
    UIToolbar *unexpectedToolbar = [[UIToolbar alloc] initWithFrame:table.bounds];
    unexpectedToolbar.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    unexpectedToolbar.barTintColor = nil;
    unexpectedToolbar.barStyle = UIBarStyleBlackTranslucent;
    [table setBackgroundView:unexpectedToolbar];
   ...
   // [table setBackgroundColor:[UIColor clearColor]];
    ...
    [revealViewController.view setBackgroundColor:[UIColor clearColor]];

//    UIColor *customTintColor = [UIColor colorWithRed:1 green:0 blue:0.5 alpha:1];
//    UIColor *customTintColor = [UIColor orangeColor];
    UIColor *customTintColor = nil;
    [revealViewController.view setTintColor:customTintColor];
    [self.view setTintColor:customTintColor];
}
...
- (NSUInteger)_controllerSectionForModelSection:(NSUInteger)section
{
    ...
    if ( !_isShowingUserSettings && section > kSectionGeneral) controllerSection -= 1;
...
//    }
    ...
    if ( section > TotalSectionsInTable )
        return NSNotFound;
    
    NSInteger totalRows = 0;
    switch ( section )
    {
        case kSectionGeneral: totalRows = TotalRowsInGeneralSection; break;
        case kSectionLocalStorage: totalRows = TotalRowsInLocalStorageSection; break;
        case kSectionFileServer: totalRows = 0; break;
        case kSectionExtFileCategories: totalRows = TotalRowsInExtFileCategoriesSection; break;
        case kSectionRemoteStorage: totalRows = TotalRowsInRemoteStorageSection; break;
        case kSectionTableEmpty: totalRows = TotalRowsInGeneralSection; break;
        case kSectionPickers: totalRows = TotalRowsInPickersSection; break;
    }
    
    if ( controllerRow >= totalRows )
        controllerRow = NSNotFound;
...
{
    NSUInteger offset = 0;
    if ( !_isShowingUserSettings && controllerSection+offset >= kSectionGeneral) offset += 1;
...
- (void)_resetFilesSectionForFileCategory:(FileCategory)category animated:(BOOL)animated
{
    ...
    row = [self _controllerRowForModelRow:row inSection:section];
    ...
    if ( row != NSNotFound )
    {
        ...
        [tableView reloadRowsAtIndexPaths:[NSArray arrayWithObject:indexPath] withRowAnimation:tableAnimation];
...
- (void)_establishSectionsForCurrentUserAnimated:(BOOL)animated
{
    ...
    {
        if ( kSectionGeneral < TotalSectionsInTable )
        {
            NSUInteger index = [self _controllerSectionForModelSection:kSectionGeneral];
...
- (void)_setCurrentDocumentName
{
    ...
    if ( name.length == 0 )
    {
        name = NSLocalizedString(@"No Project", nil);
        font = [UIFont italicSystemFontOfSize:[font pointSize]];
    }
    
    ControlViewCellContentView *cellContentView = currentProjectCell.cellContentView;
    [cellContentView setMainTextFont:font];
    [currentProjectCell setMainText:name];
}
...
- (void)askProfileDataIfNeeded
{
    if ( [cloudKitUser() requiresProfileData] )
    {
        AccountTableControllerType type = kAccountControllerUpdateICloud;
        UIViewController *modalViewController = [[EditAccountTableController alloc] initWithUsername:nil type:type];
        [self _presentViewController:modalViewController];  // <- pot ser nil
    }
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)controllerSection
{
    ...
    switch ( section )
    {
        case kSectionGeneral:
            number = TotalRowsInGeneralSection;
            break;
...
//- (NSString*)tableView:(UITableView *)tableView XXXXtitleForHeaderInSection:(NSInteger)section
//{
//    NSString *str ;
//    
...
- (UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    {
        case kSectionGeneral:
            str = NSLocalizedString( @"GENERAL", nil ) ;
            break ;
            
        case kSectionLocalStorage:
            str = NSLocalizedString( @"LOCAL STORAGE", nil ) ;
            break ;
...
//            break ;
        ...
        case kSectionFileServer: 
            str = NSLocalizedString( @"EMBEDEED WEB SERVER", nil ) ;
            break ;
    ...
    [header.textLabel setText:str];
    
    [header.textLabel setTextColor:[UIColor whiteColor]];
    header.contentView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0.1 alpha:1];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        if ( row == kAutomaticLoginRow ) cell = [self automaticLoginCell];
        else if ( row == kCurrentAccountRow ) cell = [self currentAccountCell];
        else if ( row == kRedeemCodeRow ) cell = [self redeemCodeCell];
        else if ( row == kManageAccountsRow ) cell = [self manageAccountsCell];
        else if ( row == kSettingsRow ) cell = [self settingsCell];
    }
    
    else if ( section == kSectionLocalStorage )
    {
        if ( row == kRowSourcesCurrentProject ) cell = [self currentProjectCell];
        else identifier = DisclosureCellIdentifier ;
    }
    
    else if ( section == kSectionTableEmpty )
    {
    ...
    {
        if ( row == kRowFileServer ) cell = [self fileServerSwitchCell] ;
        if ( row == kRowFileServerInfo ) cell = [self fileServerInfoCell] ;
    }
    ...
    if ( cell )
    {
        [cell.cellContentView setMainTextColor:[UIColor whiteColor]]; //nnn
        [cell.cellContentView setMainTextHighlightColor:[UIColor whiteColor]]; //nnn
        [cell.cellContentView setBottomTextHighlightColor:[UIColor whiteColor]]; //nnn
        return cell;
    }
    ...
    if ( cell == nil )
    {
        ...
        if ( identifier == DisclosureCellIdentifier )
        {
            ...
            [self _setDefaultSelectionStyleForCell:cell];
...
//            [labelView setTextColor:[UIColor whiteColor]] ;
            ...
            [labelView setRgbTintColor:Theme_RGBA(0, 255, 255, 255, 32)] ;
            [labelView setTextColor:[UIColor blackColor]] ;
            
            [labelView setTextAlignment:NSTextAlignmentCenter] ;
            [labelView setFont:[UIFont boldSystemFontOfSize:14]] ;
            [labelView setShadowOffset:CGSizeZero];
...
//    [header.textLabel setTextColor:[UIColor whiteColor]];
//    //header.tintColor = [UIColor colorWithRed:0 green:0 blue:0.1 alpha:1];
//    header.contentView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0.1 alpha:1];
//
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    if ( section == kSectionTableEmpty ) return [self messageTableEmpty];
    
    return nil;
    
////
    if ( section == kSectionGeneral ) return [self messageViewUser];
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section
{
    ...
    if ( section == kSectionTableEmpty ) return [[self messageTableEmpty] getMessageHeight] ;
...
////
    CGFloat height = 0;
    if ( section == kSectionGeneral ) height = [[self messageViewUser] getMessageHeight] ;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ( section == kSectionGeneral )
    {
        if ( row == kCurrentAccountRow )
        {
            ...
            {
                AccountTableControllerType type = kAccountControllerUpdateICloud;
                modalViewController = [[EditAccountTableController alloc] initWithUsername:nil type:type];
            }
            else
            {
                UITableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
                [self presentNoUserAlertFromView:cell];
                [tableView deselectRowAtIndexPath:indexPath animated:YES];
            }
...
//                [actionSheet showFromRect:cell.bounds inView:cell animated:YES];
                ...
                UIAlertAction *actionCancel = [UIAlertAction actionWithTitle:cancelTitle style:UIAlertActionStyleCancel
                handler:^(UIAlertAction *action)
                {
                    common();
                }];
    
                UIAlertAction *otherAction = [UIAlertAction actionWithTitle:otherButton style:UIAlertActionStyleDefault
                handler:^(UIAlertAction *action)
                {
                    common();
                    [self tableView:tableView didSelectRowAtIndexPath:[NSIndexPath indexPathForRow:kRowSourcesCategory inSection:kSectionLocalStorage]];
...
- (void)_presentViewController:(UIViewController*)modalViewController
{
...
- (void)revealController:(SWRevealViewController *)revealController animateToPosition:(FrontViewPosition)position
{
    ...
    CGFloat theExtension = position>FrontViewPositionLeft && position < FrontViewPositionRightMost ? 1.0 : 0.0;
    [toolViewController setClipExtensionPercent:theExtension];
}
